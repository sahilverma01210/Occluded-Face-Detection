...
public class CameraSource {
  ...
  private DetectorInterface frameProcessor;
  /**
   ...
   */
  private final Map<byte[], byte[]> bytesToByteBuffer = new IdentityHashMap<>();
    ...
    void setNextFrame(byte[] data, Camera camera) {
      synchronized (lock) {
        if (pendingFrameData != null) {
          camera.addCallbackBuffer(pendingFrameData);
  ...
  void setMachineLearningFrameProcessor(Activity activity) {
    synchronized (processorLock) {
      ...
      frameProcessor = new FaceDetector(activity);
    }
  }
...
//    }
    ...
    bytesToByteBuffer.put(byteArray, byteArray);
    return byteArray;
  }

  /** Called when the camera has a new preview frame. */ // For Detection of each Frame
